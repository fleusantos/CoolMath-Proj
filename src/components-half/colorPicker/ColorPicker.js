...
const ColorPicker = ({ cat, obj, changeColor, config, style, close, playBtnClickSound }) => {
  const hClose=()=>{
    close()
    playBtnClickSound()
  }
 return <>
   <div className='color-picker' style={style}>
    <img src={closeIcon} onClick={hClose} id='close' />
    <div className='bgr'>
     <Lazy src={bg} placeholder={placeholder} />
    </div>
    <div className='main'>
     {config.color[cat]?.options.map((color, index) =>
